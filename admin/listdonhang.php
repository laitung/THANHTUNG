      ...
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            ...
            <br>
            <h3 class="box-title">Tìm kiếm đơn hàng chưa xử lý theo ngày</h3>
            <form action="index.php?module=timkiemdonhangtheongay" method="POST" accept-charset="utf-8">
                <label>Ngày cần tìm</label>
                <input class="hangbanchay" type="date" name="dau" value="<?php if(isset($_POST['dau'])){ echo $_POST['dau']; } ?>" >
                <a href="index.php?module=timkiemdonhangtheongay"><button type="submit" name="submit">Kiểm tra</button></a>
              </form>
            <?php
                $sql_chitiet1 = "select * from oder where status = 'Đang xử lý'";
                ...
                while($dong_chitiet1 = mysqli_fetch_array($query1)) {
                  ...
                  echo '<br>';
                  echo "Có ".$dem1." đơn hàng đang chờ xử lý";
                 ?>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              <table id="" class="table table-bordered table-hover">
                ...
                <tbody>
                  <?php 
                        ...
                        ?>
                <?php
                $sql_chitiet = "select * from oder ORDER BY order_id DESC LIMIT $trang1,10";
                ...
                <select class="form-control select2 select2-hidden-accessible" name="trangthai_<?php echo $dong_chitiet['order_id'] ?>" style="width: 120px;" >
                  ...
                  <td><a href="index.php?module=chitietdonhang&id=<?php echo $dong_chitiet['order_id'] ?>&ten=<?php echo $dong_chitiet['fullname'] ?>&sdt=<?php echo $dong_chitiet['phone'] ?>&diachi=<?php echo $dong_chitiet['address'] ?>&makh=<?php echo $dong_chitiet['makh'] ?> ">C.tiết</a>|<a href="index.php?module=huydonhang&id=<?php echo $dong_chitiet['order_id'] ?>" onclick="return deleteAction();">Hủy</a>
                ...
                if($daxuly >= 0){
                $dong['banroi'] += $dong_chitiet['sl'];
                $update3 = mysqli_query($conn,"UPDATE chitietsp SET soluong='$daxuly',banroi='".$dong['banroi']."',date_xuly='$date' WHERE ten_sp='".$dong_chitiet['product_id']."' ") or die (mysql_error());
                $update4 = mysqli_query($conn,"UPDATE order_detal SET daban='".$dong['banroi']."' WHERE product_id='$tensp' ") or die (mysql_error()); }
                else {
                  if($daxuly < 0){
              ...
              Trang:
               <?php 
$sql_trang = mysqli_query($conn,"SELECT * FROM oder ");
